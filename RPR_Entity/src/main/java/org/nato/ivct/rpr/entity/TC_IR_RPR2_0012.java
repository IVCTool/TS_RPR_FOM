 ...
 *
 * SuT shall not update non-applicable PhysicalEntity Attributes as specified in
 * Domain Appropriateness table in SISO-STD-001-2015."
 */

/*
 * Table 7 Domain Appropriateness for PhysicalEntity Attributes e.g Attribute
 * Name Aircraft AmphibiousVehicle GroundVehicle Spacecraft
 * ----------------------------------------------------------------------------
 * CamouflageType Yes DamageState Yes EngineSmoke Yes FirePowerDisabled
 * FlamesPresent Yes Immobilized Yes IsConcealed PowerPlantOn Yes
 * SmokePlumePresent Yes TentDeployed TrailingEffectsCode Yes
 * 
 * 
 ...
 * 
 * we have to get the Classname (physicalEntity) of the ObjectInstance with
 * which the Attributes are received by reflectAttributeValues and check in a
 * List if for this classname the received attribute is one of the 'special'
 * Attributes
 * 
...
public class TC_IR_RPR2_0012 extends AbstractTestCaseIf {
    ...
    // TODO complete for all Physical Entities these are the Entities listed in
    // Table 2
    ...
    String[] naListSpacecraft = new String[] { "EngineSmokeON", "FirePowerDisabled", "IsConcealed", "TentDeployed",
            "TrailingEffectsCode" };
    String[] naListSubmersibleVessel = new String[] { "EngineSmokeON", "FirePowerDisabled", "IsConcealed",
    ...
    // HashMap with classnames as key and an array with non-applicable
    // PhysicalEntity Attributes for this class
    // to allocate the objectclassnames with a List of non-applicable PhysicalEntity
    // Attributes for this Physical Entity
    ...
    public TC_IR_RPR2_0012() {
        ...
        classNamesAndNaAttributList.put("HLAobjectRoot.BaseEntity.PhysicalEntity.Platform.AmphibiousVehicle",
                naListAmphibiousVehicle);
        classNamesAndNaAttributList.put("HLAobjectRoot.BaseEntity.PhysicalEntity.Platform.GroundVehicle",
                naListGroundVehicle);
        classNamesAndNaAttributList.put("HLAobjectRoot.BaseEntity.PhysicalEntity.Platform.MultiDomainPlatform",
                naListMultiDomainPlatform);
        classNamesAndNaAttributList.put("HLAobjectRoot.BaseEntity.PhysicalEntity.Platform.Spacecraft",
                naListSpacecraft);
        classNamesAndNaAttributList.put("HLAobjectRoot.BaseEntity.PhysicalEntity.Platform.SubmersibleVessel",
                naListSubmersibleVessel);
        classNamesAndNaAttributList.put("HLAobjectRoot.BaseEntity.PhysicalEntity.Platform.SurfaceVessel",
                naListSurfaceVessel);
    }
    ...
    class TestCaseAmbassador extends NullFederateAmbassador {
            ...
            try {
                ...
                for (Platform p : platformWorkList) {
                    if (p.getHlaClassName().equals(rtiAmbassador.getObjectClassName(theObjectClassH))) {
                        logger.debug("# discoverObjectInstance: reveived Instance with rti-ObjectClassName:  "
                        ...
                        for (ObjectInstanceHandle iH : objectInstanceHandlesAndobjectClassHandle.keySet()) {
                            logger.debug("#discoverObjectInstance: is something in objectInstanceHandlesAndobjectClassHandle ? : "
        ...
        public void reflectAttributeValues(ObjectInstanceHandle theObjectInstanceH,
            ...
            try {
                ...
                ObjectClassHandle tempLocalObjectClasshandle = objectInstanceHandlesAndobjectClassHandle
                        .get(theObjectInstanceH);
                ...
                logger.debug("# reflectAttributeValues: rti-objectInstanceName of theObject: "
                        + rtiAmbassador.getObjectInstanceName(theObjectInstanceH)); // Debug
                logger.debug("# reflectAttributeValues: rti-ObjectClassenName for this ObjectInstanceHandle : "
                        + rtiAmbassador.getObjectClassName(tempLocalObjectClasshandle)); // Debug
                logger.debug("# reflectAttributeValues: AttributHandleValueMap \"theAttributes\":  " + theAttributes); // Debug
                logger.debug(
                        "# reflectAttributeValues: the Keys in AttributHandleValueMap  : " + theAttributes.keySet());
                ...
                objectClassNamesAndReceivedAttributeList.put(tempLocalObjectClassname,
                        localListOfReceivedAttributNames);
                // now we have a allocation table ObjectClassNames and Attributlists in
                // ObjectClassNamesAndAttributeList to work with it in performTest

            } catch (FederateNotExecutionMember | NotConnected | RTIinternalError | AttributeNotDefined
                    | InvalidAttributeHandle | InvalidObjectClassHandle | ObjectInstanceNotKnown e) {
            } 
        }
    }
    ...
    protected void preambleAction(Logger logger) throws TcInconclusiveIf {
        ...
        try {
            ...
            URL[] fomList = new FomFiles().addRPR_BASE().addRPR_Enumerations().addRPR_Foundation().addRPR_Physical()
                    .addRPR_Switches().get();
    ...
    protected void performTest(Logger logger) throws TcInconclusiveIf, TcFailedIf {
        ...
        try {
            ...
            // platformWorkList = new Platform[] {new Aircraft() , new AmphibiousVehicle(), new GroundVehicle(),
                                                 //new MultiDomainPlatform(), new Spacecraft(), new SubmersibleVessel(), new SurfaceVessel() } ;
                        ...
                        for (String receivedAttribut : tempReceivedAttributList) {
                            ...
                            for (String nonAppAttribute : tempNonApplicableAttributList) {
                                ...
                                if (receivedAttribut.equals(nonAppAttribute)) {
                                    ...
                                    resultMapClassNamesAndReceivedNaAttributes.put(tempObjectClassname,
                                            resultListReceivedNaAttributes);
                                }
                            }
                        }
                    }
        ...
        for (String tempClassname : resultMapClassNamesAndReceivedNaAttributes.keySet()) {
            failMessage = failMessage + " " + tempClassname + " updated "
